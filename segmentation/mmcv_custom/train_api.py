...
from .checkpoint import load_checkpoint_partial, load_checkpoint_pretrain
        ...
        if isinstance(cfg.load_from, dict):
            if cfg.load_from.type == 'partial_load':
                load_checkpoint_partial(
                    runner.model,
                    cfg.load_from.path,
                    not_load=cfg.load_from.not_load,
                    logger=logger
                )
            elif cfg.load_from.type == 'load_pretrain':
                load_checkpoint_pretrain(
                    runner.model,
                    cfg.load_from.path,
                    map_pairs=cfg.load_from.map_pairs,
                    not_load=cfg.load_from.not_load,
                    logger=logger
                )
            else:
