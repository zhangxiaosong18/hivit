...
from timm.data import Mixup, ImageDataset
...
def build_loader(config):
    ...
    dataset_val = torch.utils.data.Subset(dataset_val, tuple(range(100)))
    print(f"local rank {config.LOCAL_RANK} / global rank {rank} successfully build val dataset")

    num_tasks = size
    global_rank = rank
    if config.DATA.ZIP_MODE and config.DATA.CACHE_MODE == 'part':
        indices = np.arange(rank, len(dataset_train), size)
    ...
    indices = np.arange(rank, len(dataset_val), size)
...
def build_dataset(is_train, config):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        if config.DATA.ZIP_MODE:
            ...
            #     os.path.join(config.DATA.DATA_PATH, prefix + '.tar'),
            #     transform=transform,
            # )
